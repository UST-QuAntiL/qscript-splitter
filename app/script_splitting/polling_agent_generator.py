...
def generate_polling_agent(parameters, return_values):
    ...
    for inputParameter in parameters:
        load_data_str += '\n'
        load_data_str += '                    if variables.get("' + inputParameter + '").get("type") in ["String", "Number", "Boolean"]:\n'
    ...
    for outputParameter in return_values:
        ...
        outputHandler += '                    elif isinstance(' + outputParameter + ', int) or isinstance(' + outputParameter + ', float):\n'
        outputHandler += '                        print("OutputParameter (int / float) %s" % ' + outputParameter + ')\n'
        outputHandler += '                        body["variables"]["' + outputParameter + '"] = {"value": ' + outputParameter + ', "type": "Number"}\n'
